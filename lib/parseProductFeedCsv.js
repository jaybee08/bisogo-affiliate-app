...
export async function fetchAndParseProductFeedCsv() {
  try {
    ...
    // Filter out products with Availability: 'OutOfStock'
    const filteredData = data.filter(product => product.Availability !== 'OutOfStock');

    console.log('Parsed DataFeed:', filteredData);
    return filteredData;
  } catch (error) {
